...
public class AssessActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!answrd){
                    if(radbt1.isChecked() && !radbt2.isChecked() && !radbt3.isChecked()
                            && !radbt4.isChecked()&& !radbt5.isChecked()){
                        scores += 1;
                        checkAns();
                    }
                    else if(radbt2.isChecked() && !radbt1.isChecked() && !radbt3.isChecked()
                            && !radbt4.isChecked() && !radbt5.isChecked()){
                        scores += 2;
                        checkAns();
                    }
                    else if(radbt3.isChecked() && !radbt1.isChecked() && !radbt2.isChecked()
                            && !radbt4.isChecked() && !radbt5.isChecked()){
                        scores += 3;
                        checkAns();
                    }
                    else if(radbt4.isChecked() && !radbt1.isChecked() && !radbt3.isChecked()
                            && !radbt2.isChecked() && !radbt5.isChecked()){
                        scores += 4;
                        checkAns();
                    }
                    else if(radbt5.isChecked() && !radbt1.isChecked() && !radbt3.isChecked()
                            && !radbt2.isChecked() && !radbt4.isChecked()){
                        scores += 5;
                        checkAns();
                    }else {
    ...
    private void checkAns(){
        answrd = true;
        RadioButton rbSelected = findViewById(radGrop.getCheckedRadioButtonId());
        int answerNr = radGrop.indexOfChild(rbSelected) + 1;
